...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final expiringItems = await itemRepo.getExpiringItems(7);
  ...
  Future<void> _showBulkMoveDialog() async {
    ...
    final selectedLocationId = await showDialog<String?>(
      context: context,
      builder: (context) => LocationPickerDialog(locations: locations),
    );
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: _items.length + (_expiringItems.isNotEmpty ? 1 : 0),
        itemBuilder: (context, index) {
          // Show "Expiring Soon" header if we have expiring items
          if (_expiringItems.isNotEmpty && index == 0) {
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  ...List.generate(_expiringItems.length, (i) {
                    final item = _expiringItems[i];
                    ...
                    return ItemCard(
                      ...
                      onTap: () {
                        if (_isSelectionMode) {
                          ...
                          _navigateToItemDetails(item.id);
                        }
                      },
                      onLongPress: () {
                        if (!_isSelectionMode) {
          ...
          // Adjust index for items list when we have expiring items section
          final itemIndex = _expiringItems.isNotEmpty ? index - 1 : index;
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ChangeNotifierProvider(
          create: (context) =>
              SearchViewModel(itemRepository: context.read<ItemRepository>()),
  ...
  Future<void> _navigateToItemDetails(String itemId) async {
    // Use go_router for declarative navigation with push (maintains navigation stack)
    if (itemId.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Invalid item ID')));
      return;
    }

    // Navigate and wait for result
    final result = await context.push<bool>('/item/$itemId');

    // If item was deleted or modified, reload data
